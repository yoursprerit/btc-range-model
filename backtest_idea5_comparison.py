"""Idea 5 Entry Signal Comparison — MSTR & MSTU

Compares the CURRENT entry gate where U1 fires whenever:
    ehma3 > 0.7%  AND  hb3 >= 2

vs IDEA 5 — 5-day error average confirmation: require the 5-bar rolling
average of high-side prediction errors to also be positive (> 0.3%):
    ehma5[i] = mean(err_hi[i-4 : i+1])
    u1 = (ehma3 > 0.7) & (hb3 >= 2) & (ehma5 > 0.3)

If ehma3 > 0.7% but ehma5 ≤ 0.3%, the 3-day spike is happening against
a flatter/negative 5-day background — typical of a bear bounce embedded
in a broader downtrend.  Requiring the 5-day context to also be positive
demands a broader structural basis before entry is triggered.
The XOR trend gate is unchanged; only U1 gets the wider-window filter.
...
def compute_signals(comp: pd.DataFrame, entry_mode: str):
    """Return dict of signal arrays. entry_mode: 'current' or 'idea5'."""
    N       = len(comp)
    ...
    # Idea 5: 5-day error average — provides broader structural context
    ehma5 = np.array([
...
def main():
    print("\n" + "═"*70)
    print("  Idea 5 Comparison — MSTR & MSTU (all UI periods)")
    print("  U1 filter: 5-day error average ehma5 > 0.3% (broader context)")
    print("═"*70)
    ...
    for asset_label, ticker, stop_pct, periods in configs:
        ...
        for period_name, (end_iso, start_iso) in periods.items():
            ...
            r_i5  = run_asset_backtest(preds_df, raw_df, ticker, start_iso, end_iso, stop_pct, "idea5")
            if r_cur and r_i5:
                delta_ret = r_i5["strat_ret"] - r_cur["strat_ret"]
                delta_dd  = r_i5["max_dd"]    - r_cur["max_dd"]
                delta_sh  = r_i5["sharpe"]    - r_cur["sharpe"]
                ...
                print(f"  {'':8}  {'Δ idea5':<9}"
                      ...
                      f"  {'':>6}"
                      f"  {r_i5['n_trades']-r_cur['n_trades']:>+4}"
                      f"  {r_i5['n_sl']-r_cur['n_sl']:>+4}")
            print()
