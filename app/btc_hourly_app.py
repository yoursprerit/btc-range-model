...
def compute_alltime_cone_14d_metrics(end_date_iso):
    ...
    test_start_str = meta.get("test_start", "2025-11-25")
...
<div style='background:#eff6ff; border:2px solid #2563eb; border-radius:12px;
  ...
  <div style='font-size:11px; color:#3b5280; line-height:1.8;'>
    ...
    &nbsp;·&nbsp;
    ⚠️ Data before Nov 25, 2025 is <b>in-sample</b> (CT model training + validation period)
  </div>
    ...
    OOS_START  = pd.Timestamp("2025-11-25")   # H/L test_start (after val embargo)
    cutoffs    = _training_cutoffs()
    ct_cutoff  = cutoffs.get("daily H/L")
    ct_str     = ct_cutoff.strftime("%b %d, %Y") if ct_cutoff else "Sep 24, 2025"
    s_oos      = None
...
def render_retrain_dashboard():
    ...
    def _age_status(key, test_end_str=None):
        # Age = days since test_end (how much new data the model hasn't seen yet).
        # Falls back to train_end if test_end is not stored in the artefact.
        train_end = cutoffs.get(key)
        if test_end_str:
            te_for_age = pd.Timestamp(test_end_str).normalize()
        elif train_end is not None:
            te_for_age = train_end
        else:
            return None, "⚪", "unknown age"
        warn = WARN_D.get(key, 56)
        over = OVERDUE_D.get(key, 56)
        age  = int((today_utc - te_for_age).days)
        if age >= over:
            return age, "🔴", f"{age}d since OOS end  (limit: {over}d)"
        if age >= warn:
            return age, "🟡", f"{age}d since OOS end  (warn: {warn}d)"
        return age, "🟢", f"{age}d since OOS end  (limit: {over}d)"
