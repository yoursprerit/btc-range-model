...
# ═══════════════════════════════════════════════════════════════════
# BTC Backtesting  (trade BTC spot directly)
# ═══════════════════════════════════════════════════════════════════
...
def run_btc_backtest(end_date_iso: str,
                     ...
                     logic_version: str = _BT_LOGIC_VERSION,
                     entry_gate: str = "pure_regime"):
    """BTC backtest driven by BTC TF2+V-Gate signals — trades BTC spot directly.

    Identical signal logic to run_mstr_backtest but executes in BTC instead of MSTR.
    BTC prices come from raw_df["btc_close"] and raw_df["btc_low"] — no yfinance fetch.
    Uses a 5% fixed stop-loss (vs 3% for MSTR) reflecting BTC's higher volatility.
    """
    WARMUP = 35
    ...
    # BTC execution prices from raw_df (no yfinance needed)
    btc_px = btc_closes.reindex(dates).ffill().bfill().values.astype(float)
    btc_lo = btc_lows.reindex(dates).ffill().bfill().values.astype(float)  # noqa: F841
    ...
    for i in range(N):
        ...
        if pos == "LONG":
            cur = btc_qty * price
            if price <= stop_px:                   # triggered on close
                exit_px  = price
                ...
                # Same-bar exit: signal and fill on the same daily close
                should_exit = bool(d3[i] or (d2[i] and not bull_regime[i]))
                exit_lbl    = "D3" if d3[i] else "D2 (bear)"
                if should_exit:
                    nav = cur
            ...
            if from_sl:
                bars_since_sl += 1
            # Same-bar signals: entry and exit both use current bar
            _exit_at_i = d3[i] or (d2[i] and not bull_regime[i])
            # SL5: in BULL regime re-enter immediately; in BEAR/neutral wait 10 bars
            _sl_reentry_ok = (not from_sl
                              or bool(bull_regime[i])
    ...
    last_price = float(btc_px[N-1])
    if pos == "LONG" and np.isfinite(last_price) and last_price > 0:
        nav_arr[N-1] = btc_qty * last_price
...
def _run_fixed_period_btc_backtest(end_date_iso: str, backtest_start_iso: str,
    ...
    Identical to _run_fixed_period_mstr_backtest but calls run_btc_backtest.
    Cache key includes end_date_iso + backtest_start_iso + model_mtime + data_end +
    logic_version + entry_gate so it auto-invalidates on any
    backtest logic change (logic_version), or entry_gate variant.
    """
...
def render_btc_trading_strategy_dashboard(bt_bear, bt_bull, bt_full_oos=None,
    ...
    Same four-period layout as render_mstr_trading_strategy_dashboard but with:
      • Orange/amber color scheme to distinguish from MSTR (purple) results
      • BTC spot prices in trade log (no split-adjust needed)
      • B&H comparison uses BTC rather than MSTR
    """
    st.markdown("---")
    st.subheader("₿ BTC — BTC Signal-Driven Backtest (TF2 + V-Gate)")
    ...
    _btc_footer = """
  <div style='border-top:1px solid #fdba74; margin:10px 0;'></div>
  <!-- EXIT -->
  <div style='margin-bottom:12px;'>
    <div style='font-size:11px; font-weight:700; color:#9a3412; text-transform:uppercase;
  ...
  <div style='border-top:1px solid #fdba74; margin:10px 0;'></div>
  <div style='font-size:11px; color:#9a3412; line-height:1.8;'>
    ⏱ <b>Execution:</b> BTC signal and execution both on same daily close
    &nbsp;·&nbsp;
    💰 <b>Capital:</b> $100,000 initial
    &nbsp;·&nbsp;
...
<div style='background:#fff7ed; border:2px solid #ea580c; border-radius:12px;
  ...
  <div style='background:#ffedd5; border-radius:8px; padding:10px 14px;
    ...
    engine and trade BTC spot directly. Entry/exit are driven by BTC CT model predictions
    so signal and execution are on the same asset.
  </div>
...
<div style='background:#fff7ed; border:2px solid #ea580c; border-radius:12px;
  ...
  <div style='background:#fff7ed; border:1px solid #f97316; border-radius:7px;
       padding:8px 13px; margin-bottom:12px; font-size:12px; color:#c2410c;'>
        ...
        for bt_src, lbl in [(bt_bear, "bear"), (bt_bull, "bull"),
            ...
            with ltabs[lt_idx]:
                ...
                if lbl == "oos":
                    ...
                    caption += "⚠️ pre-Sep 2025 = in-sample for BTC CT model."
                st.caption(caption)
            lt_idx += 1
...
# ════════════════════════════════════════════════════════════════════════
# Tabs: Live | Historical | BTC | MSTR | MSTU | MSTR Options | MSTU Options | Explainability | Leading Indicators | Retrain Status
# ════════════════════════════════════════════════════════════════════════
...
with tab_btc:
    st.markdown("## ₿ BTC — BTC Signal-Driven Backtesting")
    st.markdown(
        "Runs the **same TF2 + V-Gate strategy** on **BTC itself** — both signals and "
        "execution are in BTC spot. Entry/exit are driven by BTC CT model predictions. "
