...
_BT_LOGIC_VERSION = "sl8-v39-mstr-nostop-mstu6-vrecent5-reentry-override"
...
# All three assets share the SAME entry gate — Standard MA (above-MA30) — plus the
# SAME thresholds (U1 > +1.3%, D2 < −1.3%) and the SAME regime-adaptive D2/D3 exit;
# only the per-asset STOP differs (below). U1 was raised +0.9% → +1.3% (2026-07d) to minimize bear-market losing
...
# 2026-07 retune: all three assets trade the Standard MA (above-MA30) entry gate —
# U1 AND ((above_MA30 XOR Clean 7d) OR V-rev). On the current data it is the most
...
#   MSTR → Standard MA · no stop     (Full +266% vs B&H +4%; Bull +142%; Bear +57%)
#   MSTU → Standard MA · fixed −6%   (Full +524% vs B&H −87%; Bull +229%; Bear +101%)
# These are the default-selected options in the BTC/MSTR/MSTU/options radio buttons.
# 2026-07e per-asset STOP retune (k-fold + synthetic-OOS validated — see the stop-loss
# eval): MSTR's fixed −3% only ever whipsawed it out during bull pullbacks (it sat at
# ~0.6σ and a 1× name has no wipeout tail), so MSTR now runs SIGNAL-EXIT-ONLY with no
# fixed stop — Full +212→+266%, Sharpe 1.34→1.41, MDD unchanged −22%, win-rate 75→85%.
# MSTU's −3% sat at only ~0.3σ on a 2× fund and stopped out on routine noise; widening
# to a vol-matched −6% (≈2× MSTR's old 3%) lifts Full +309→+524%, Sharpe 1.08→1.27,
# MDD −48→−42%, win-rate 33→62%, while still truncating the crash tail that going
# stopless opens up (P(MDD<−80%) climbs toward ~44% stopless vs ~26% at 6–8%).
...
#       above MA30 re-admits the STOPPED sleeve even if the XOR block would veto. With
#       the 2026-07e retune only MSTU carries a stop, so the override now applies to MSTU
#       alone; MSTR (no stop) never sets from_sl and is unaffected.
...
# Net vs pre-retune: MSTR Full +266%, MSTU Full +524%, BTC +86%. Bear and OOS improve
# (MSTU Bear +61→+101%, OOS +95→+144%); the only trade-off is a slightly deeper MSTU
# Bull drawdown (−27→ still −27% at 6%; wider stops deepen it).
...
# 2026-07f — ETHA (iShares Ethereum Trust, 1× spot-ETH ETF) added as a fourth sleeve
# on the same BTC signal with the MSTR treatment: Standard MA gate, signal-exit-only,
# no fixed stop. Trades from the fund's 2024-07-23 inception. Validated in
# ETHA_BMNR_STRATEGY_EVAL.md (fixed stops ≤8% only hurt, exactly as on MSTR).
ETHA_STRATEGY_GATE  = "above_ma30"
ETHA_STRATEGY_LABEL = "Standard MA"
ETHA_STOP_PCT = None   # ETHA: no fixed stop (signal exits only) — 2026-07f add
ETHA_INCEPTION_ISO = "2024-07-23"   # first ETHA trading day — sleeves never start earlier
...
# capitulation low, then locked out of the recovery" failure that made the sleeve
# miss the mid-Apr→mid-May 2025 rally. Fires only after a fixed-stop exit, so with the
# 2026-07e retune it applies to MSTU alone — BTC and MSTR carry no stop (from_sl is
# never set) and are unaffected. Surgical; stable across a 12–20 bar window.
...
# Clean-Breakout window is visible, with 🔻 D1/D2/D3 + 🔼 U1 markers over it.
...
def fetch_equity_prices():
    """Fetch live MSTR, MSTU, ETHA, STRC and SATA prices via Yahoo Finance."""
    prices, changes = {}, {}
    for ticker in ("MSTR", "MSTU", "ETHA", "STRC", "SATA"):
        try:
    ...
    return (prices.get("MSTR"), changes.get("MSTR"),
            ...
            prices.get("ETHA"), changes.get("ETHA"),
...
mstr_price, mstr_chg, mstu_price, mstu_chg, etha_price, etha_chg, strc_price, strc_chg, sata_price, sata_chg = fetch_equity_prices()
...
def compute_trend_signatures(target_date_iso: str, data_end=None,
    ...
    _d3_hist = np.zeros(n, dtype=bool)
    _u1_hist = np.zeros(n, dtype=bool)
    _hi_run = 0   # consecutive hi_breaks ending at the previous bar
    for _i in range(n):
        _s = max(0, _i - 2)
        ...
        _d3_hist[_i] = (_hi_run >= 3) and bool(lo_break[_i])
        _u1_hist[_i] = (_ehma_i > U1_ERRHI_MIN) and (_hb3_i >= 2)
        _hi_run = _hi_run + 1 if hi_break[_i] else 0
    ...
    # 2026-07 retune: live/historical entry signal = the BTC strategy gate
    # (BTC_STRATEGY_GATE, currently Standard MA for BTC/MSTR/MSTU). Matches every
    # backtest exactly so live = historical = backtest.
    #   Standard MA  = U1 AND ((above_MA30 XOR Clean 7d) OR V-rev)
    #   Pure Regime  = U1 AND (Bull Regime OR (Clean 7d & below MA30) OR V-rev)
    if BTC_STRATEGY_GATE == "above_ma30":
        tf1_triggered = u1_triggered and ((bool(above_ma30) != bool(clean_10d)) or v_gate_ok)
    else:
...
def run_full_period_backtest(end_date_iso: str,
    ...
    if BTC_STRATEGY_GATE == "above_ma30":
        tf1_entry = u1 & ((above_ma30 ^ clean_10d) | v_recent)          # Standard MA (XOR gate)
    else:
        tf1_entry = u1 & (bull_regime | (clean_10d & ~above_ma30) | v_recent)  # Pure Regime
...
@st.cache_data(ttl=86_400)
def _load_etha_prices() -> pd.DataFrame | None:
    """ETHA daily OHLCV (post-inception 2024-07-23) from versioned CSV."""
    return _read_price_csv("etha_daily.csv")
        ...
        if pos == "LONG":
            ...
            if _has_stop and price <= stop_px:     # triggered on close (matches research evaluation)
                ...
                trades.append(dict(
                    ...
                    pnl_abs=nav-e_nav,   exit_signal=f"SL-fixed-{int(MSTR_STOP_PCT*100)}%",
            ...
            # exit, so with MSTR now stop-less (2026-07e) it is permanently inert here.
            ...
            if (tf1_entry[i] and _sl_reentry_ok and (from_sl or not _exit_at_i)) or _reentry_override:
                ...
                e_nav = nav; pos = "LONG"
                stop_px = price * (1 - MSTR_STOP_PCT) if _has_stop else 0.0
...
# ═══════════════════════════════════════════════════════════════════
# ETHA Backtesting  (iShares Ethereum Trust — BTC signals · ETHA execution; 2026-07f)
# ═══════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner="Running ETHA backtest …")
def run_etha_backtest(end_date_iso: str,
                      backtest_start_iso: str = "2024-07-23",
                      initial_capital: float = 100_000.0,
                      model_mtime: float = 0.0,
                      data_end: str = "",
                      logic_version: str = _BT_LOGIC_VERSION,
                      data_mtime: float = 0.0,
                      entry_gate: str = "above_ma30"):
    """ETHA backtest driven by BTC TF2+V-Gate signals (2026-07f add).

    Execution prices are loaded from the versioned data/backtest/etha_daily.csv
    (pulled by scripts/pull_backtest_data.py) with fallback to live yfinance.
    Signal generation uses the BTC CT model predictions (live).  ETHA trades
    with the ETHA treatment — Standard MA gate, signal-exit-only, no fixed
    stop — and never before the fund's 2024-07-23 inception (the start date is
    clamped so no back-filled pre-listing bars enter the run).
    """
    WARMUP = 35

    end_dt   = pd.Timestamp(end_date_iso)
    start_dt = pd.Timestamp(backtest_start_iso)
    # clamp to the fund's first trading day — a window opening earlier (e.g. the
    # shared "2024-06-01" full/bull periods) starts at inception instead, so the
    # ffill/bfill price alignment can never manufacture pre-listing bars.
    start_dt = max(start_dt, pd.Timestamp(ETHA_INCEPTION_ISO))
    pre_dt   = start_dt - pd.Timedelta(days=60)
    # 200 calendar days ensures dist_hi_90 is fully warm before start_dt.
    fetch_start = (start_dt - pd.Timedelta(days=200)).strftime("%Y-%m-%d")
    fetch_end   = (end_dt + pd.Timedelta(days=3)).strftime("%Y-%m-%d")
    ...
    # ── ETHA prices: versioned CSV → fallback to live yfinance ──────────────
    _etha_csv = _load_etha_prices()
    if _etha_csv is not None and "close" in _etha_csv.columns:
        _etha_csv   = _yf_extend_price_df(_etha_csv, "ETHA", end_date_iso)
        _etha_close = _etha_csv["close"].sort_index()
        _etha_low   = _etha_csv["low"].sort_index() if "low" in _etha_csv.columns else _etha_close
    else:
        try:
            d_etha = yf.download(
                "ETHA",
            ...
            if isinstance(d_etha.columns, pd.MultiIndex):
                d_etha.columns = [c[0] for c in d_etha.columns]
            d_etha.index = pd.DatetimeIndex(d_etha.index).tz_localize(None).normalize()
        except Exception:
            return None
        if d_etha.empty or "Close" not in d_etha.columns:
            return None
        _etha_close = d_etha["Close"].sort_index()
        _etha_low   = d_etha["Low"].sort_index() if "Low" in d_etha.columns else _etha_close

    _etha_all = _etha_close.reindex(
        pd.date_range(_etha_close.index[0], max(_etha_close.index[-1], end_dt), freq="D")
    ).ffill()
    _etha_lo_all = _etha_low.reindex(
        pd.date_range(_etha_low.index[0], max(_etha_low.index[-1], end_dt), freq="D")
    ).ffill()
    etha_px = _etha_all.reindex(dates).ffill().bfill().values.astype(float)
    etha_lo = _etha_lo_all.reindex(dates).ffill().bfill().values.astype(float)  # noqa: F841
    ...
    # ── Backtest loop — execute in ETHA ──────────────────────────────────────
    # 2026-07e: ETHA runs SIGNAL-EXIT-ONLY (ETHA_STOP_PCT is None). With no fixed
    # stop, `_has_stop` is False → the stop never fires, `from_sl` is never set,
    # and the post-stop re-entry override / SL5 cooldown stay inert. Exits are the
    # D2/D3 regime signals only.
    _has_stop = ETHA_STOP_PCT is not None
    nav      = initial_capital; pos = "CASH"; etha_qty = 0.0
    e_price = e_nav = e_date = e_trigger = None
    e_reentry = False   # True when current position was entered after an SL exit
    stop_px  = 0.0
    # SL5 regime-adaptive re-entry state (inert when _has_stop is False)
    ...
    for i in range(N):
        ...
        if sata_cash and pos == "CASH" and dates[i].weekday() < 5:
            nav *= SATA_DAILY_FACTOR
        if not np.isfinite(price) or price <= 0:
            nav_arr[i] = etha_qty * etha_px[i-1] if pos == "LONG" and i > 0 else nav
            continue
        if pos == "LONG":
            cur = etha_qty * price
            if _has_stop and price <= stop_px:     # triggered on close (matches research evaluation)
                exit_px  = price                   # fill at close price
                exit_nav = etha_qty * exit_px
                nav = exit_nav
                trades.append(dict(
                    entry_date=e_date,    entry_price=e_price, entry_nav=e_nav,
                    entry_trigger=e_trigger, exit_date=dates[i], exit_price=exit_px,
                    exit_nav=nav, pnl_pct=(exit_px/e_price-1)*100,
                    pnl_abs=nav-e_nav,   exit_signal=f"SL-fixed-{int(ETHA_STOP_PCT*100)}%",
                    duration_days=(dates[i]-e_date).days, stop_triggered=True,
                    was_reentry=e_reentry,
                ))
                pos = "CASH"; etha_qty = 0.0; stop_px = 0.0; e_reentry = False
                from_sl = True; bars_since_sl = 0   # SL5: start cooldown
            else:
                # Same-bar exit: signal and fill on the same daily close (after-hours execution)
                should_exit = bool(d3[i] or (d2[i] and not bull_regime[i]))
            ...
            # lock the sleeve out of. Fires only after a stop; never in normal state.
            # Override is gated on _has_stop: it can only fire after a fixed-stop
            # exit, so with ETHA now stop-less (2026-07e) it is permanently inert here.
            _reentry_override = bool(_has_stop and REENTRY_OVERRIDE_BARS and from_sl
                                     and bars_since_sl <= REENTRY_OVERRIDE_BARS
                                     and u1[i] and above_ma30[i] and not _exit_at_i)
            # Post-SL re-entries bypass the _exit_at_i gate (D2 fires in bear market
            ...
            if (tf1_entry[i] and _sl_reentry_ok and (from_sl or not _exit_at_i)) or _reentry_override:
                e_reentry = bool(from_sl)              # mark if entering after SL
                etha_qty = nav / price; e_price = price; e_date = dates[i]
                e_nav = nav; pos = "LONG"
                stop_px = price * (1 - ETHA_STOP_PCT) if _has_stop else 0.0
                from_sl = False; bars_since_sl = 0   # reset SL5 state on re-entry
                if v_recent[i]:
                    e_trigger = "U1 + V-reversal"
                elif above_ma30[i]:
                    ...
                    e_trigger = "U1 + Clean 7d"
        nav_arr[i] = etha_qty * price if pos == "LONG" else nav

    if pos == "LONG" and np.isfinite(etha_px[N-1]) and etha_px[N-1] > 0:
        nav_arr[N-1] = etha_qty * etha_px[N-1]

    nav_series = pd.Series(nav_arr[_bt0:], index=dates[_bt0:]).ffill()
    bh_px0     = etha_px[_bt0]
    bh_series  = pd.Series(
        initial_capital * etha_px[_bt0:] / bh_px0, index=dates[_bt0:])
    ...
    return _relabel_result_to_visible(dict(
        strategy   = "TF2+V-Gate (ETHA)",
        trades     = trades,
        nav_series = nav_series,
        bh_series  = bh_series,
        open_pos      = pos == "LONG",
        last_price    = float(etha_px[N-1]) if N > 0 and np.isfinite(etha_px[N-1]) else None,
        open_entry    = (dict(price=e_price, date=e_date, nav=e_nav,
                             entry_trigger=e_trigger,
                             stop_price=(round(stop_px, 4) if _has_stop else None)) if pos == "LONG" else None),
        from_sl       = from_sl,
        ...
        bull_regime_series = bull_regime_series,
        btc_price_series   = pd.Series(comp["actual_close"].values.astype(float)[_bt0:], index=dates[_bt0:]),
        etha_price_series  = pd.Series(etha_px[_bt0:], index=dates[_bt0:]),
        stats = dict(
            strategy        = "TF2+V-Gate (ETHA)",
            entry_gate      = entry_gate,
            sata_cash       = sata_cash,
...
@st.cache_data(show_spinner="Loading fixed-period ETHA backtest …")
def _run_fixed_period_etha_backtest(end_date_iso: str, backtest_start_iso: str,
                                    model_mtime: float = 0.0,
                                    data_end: str = "",
                                    logic_version: str = _BT_LOGIC_VERSION,
                                    data_mtime: float = 0.0,
                                    entry_gate: str = "above_ma30"):
    """Cached wrapper for fixed-period ETHA backtests."""
    return run_etha_backtest(end_date_iso, backtest_start_iso,
                             model_mtime=model_mtime, data_end=data_end,
                             logic_version=logic_version, data_mtime=data_mtime,
                             entry_gate=entry_gate)
...
def run_mstu_backtest(end_date_iso: str,
    ...
    # 2026-07e: MSTU keeps a fixed stop, widened −3% → −6% (vol-matched to its ~2×
    # volatility). It is the only sleeve that still carries a stop, so the post-stop
    # re-entry override applies to MSTU alone.
    _has_stop = MSTU_STOP_PCT is not None
        ...
        if pos == "LONG":
            ...
            if _has_stop and price <= stop_px:     # triggered on close (matches research evaluation)
                ...
                trades.append(dict(
                    ...
                    entry_trigger=e_trigger, exit_date=dates[i], exit_price=exit_px,
                    exit_nav=nav, pnl_pct=(exit_px/e_price-1)*100,
                    pnl_abs=nav-e_nav,   exit_signal=f"SL-fixed-{int(MSTU_STOP_PCT*100)}%",
                    duration_days=(dates[i]-e_date).days, stop_triggered=True,
                    was_reentry=e_reentry,
                ))
            ...
            # window after a stop, a fresh U1 above the MA30 re-admits even when the
            # XOR combined-block would otherwise veto. Recovers post-capitulation
            # rallies (e.g. Apr 2025) that the fixed −6% stop + XOR block otherwise
            ...
            if (tf1_entry[i] and _sl_reentry_ok and (from_sl or not _exit_at_i)) or _reentry_override:
                ...
                e_nav = nav; pos = "LONG"
                stop_px = price * (1 - MSTU_STOP_PCT) if _has_stop else 0.0
                from_sl = False; bars_since_sl = 0   # reset SL5 state on re-entry
                if v_recent[i]:
                    e_trigger = "U1 + V-reversal"
                elif above_ma30[i]:
    ...
    <table style='width:100%; border-collapse:collapse; font-size:12px; color:#1e3a8a;'>
      ...
      <tr>
        <td style='width:36px; vertical-align:top; padding:3px 6px 3px 0; font-weight:700;
             color:#2563eb;'>②</td>
        <td style='vertical-align:top; padding:3px 0;'>
          <b>Pure Regime gate</b> — Bull Regime <b>OR</b> washed-out Clean Breakout <b>OR</b> ⚡ V-reversal:
          <div style='margin:5px 0 0 4px; line-height:2.1;'>
            ...
            <br>
            <span style='background:#dbeafe; border-radius:4px; padding:1px 7px;'>🧹 Clean Breakout</span>
...
def render_mstr_trading_strategy_dashboard(bt_bear, bt_bull, bt_full_oos=None,
  ...
  <div style='border-top:1px solid #c4b5fd; margin:10px 0;'></div>
  <!-- NO STOP-LOSS (2026-07e: MSTR signal-exit-only) -->
  <div style='margin-bottom:12px;'>
    <div style='font-size:11px; font-weight:700; color:#5b21b6; text-transform:uppercase;
         letter-spacing:0.8px; margin-bottom:6px;'>🔒 Position Management (no fixed stop)</div>
    <div style='font-size:12px; color:#4c1d95;'>
      <b>No stop-loss</b> — MSTR positions are closed <b>only</b> by the shared D2/D3 regime
      exit signals (2026-07e retune). MSTR's old fixed −3% stop sat at only ~0.6σ and, on a 1×
      instrument with no wipeout tail, only ever whipsawed the sleeve out during bull-market
      pullbacks; removing it lifts <b>Full +212% → +266%</b> (Sharpe 1.34 → 1.41, drawdown
      unchanged −22%, win-rate 75% → 85%). With no stop there is no post-stop override or SL
      cooldown — the V-reversal entry and shared D2/D3 exits carry the sleeve.
    </div>
  </div>
  <div style='border-top:1px solid #c4b5fd; margin:10px 0;'></div>
...
def render_etha_trading_strategy_dashboard(bt_bear, bt_bull, bt_full_oos=None,
                                           bt_full=None, key_suffix: str = "",
                                           strategy_variant: str = "pure_regime") -> None:
    """Render the ETHA backtesting dashboard (BTC TF2+V-Gate signals → ETHA execution).

    Same four-period layout as render_mstr_trading_strategy_dashboard but with:
      • Teal color scheme to distinguish from the BTC / ETHA / MSTU results
      • ETHA ETF prices in the trade log (versioned dataset, from inception)
      • B&H comparison uses ETHA rather than BTC
    """
    st.markdown("---")
    st.subheader("🔹 ETHA — BTC Signal-Driven Backtest (TF2 + V-Gate)")

    if bt_bear is None and bt_bull is None:
        st.info("⚙️ ETHA backtest computing … fetching ETHA data and building signals. "
                "Refresh in a moment.")
        return
  ...
  <div style='margin-bottom:12px;'>
    <div style='font-size:11px; font-weight:700; color:#115e59; text-transform:uppercase;
         letter-spacing:0.8px; margin-bottom:6px;'>📤 Exit — BTC signals trigger ETHA sell</div>
    <table style='width:100%; border-collapse:collapse; font-size:12px; color:#134e4a;'>
      <tr>
        ...
        <td style='vertical-align:top; padding:3px 0;'>
          BTC above MA30 AND MA30 rising → exit ETHA on <b>D3 only</b> (patient)
        </td>
      </tr>
      <tr><td colspan='2' style='padding:4px 0;'></td></tr>
      <tr>
        ...
        <td style='vertical-align:top; padding:3px 0;'>
          BTC below MA30 or MA30 flat → exit ETHA on <b>D2 or D3</b> (defensive)
        </td>
      </tr>
    </table>
  </div>
  <div style='border-top:1px solid #5eead4; margin:10px 0;'></div>
  <!-- NO STOP-LOSS (2026-07e: ETHA signal-exit-only) -->
  <div style='margin-bottom:12px;'>
    <div style='font-size:11px; font-weight:700; color:#115e59; text-transform:uppercase;
         letter-spacing:0.8px; margin-bottom:6px;'>🔒 Position Management (no fixed stop)</div>
    <div style='font-size:12px; color:#134e4a;'>
      <b>No stop-loss</b> — ETHA positions are closed <b>only</b> by the shared D2/D3 regime
      exit signals (the ETHA treatment; 2026-07f add). A 1× spot-ETH ETF has no wipeout tail,
      and in the ETHA/BMNR evaluation every fixed stop ≤8% only reduced return and Sharpe —
      ...
      See <b>ETHA_BMNR_STRATEGY_EVAL.md</b> for the full validation.
    </div>
  </div>
  <div style='border-top:1px solid #5eead4; margin:10px 0;'></div>
  <div style='font-size:11px; color:#115e59; line-height:1.8;'>
    ⏱ <b>Execution:</b> Same-day — the BTC 12:00-UTC signal on bar <i>i</i> triggers the ETHA trade at that same bar's close (after-hours fill)
    &nbsp;·&nbsp;
    💰 <b>Capital:</b> $100,000 initial
    &nbsp;·&nbsp;
    📅 <b>ETHA prices:</b> versioned dataset, from the fund's 2024-07-23 inception (window starts are clamped there)
    &nbsp;·&nbsp;
    ⚠️ Pre-{_oos_cut_str("%b %Y")} data is <b>in-sample</b> for the BTC CT model
  </div>
</div>"""
...
<div style='background:#f0fdfa; border:2px solid #0d9488; border-radius:12px;
  ...
  <div style='font-size:14px; font-weight:700; color:#134e4a; margin-bottom:14px;
       letter-spacing:0.3px;'>
    📊 TF2 + V-Gate on ETHA &nbsp;—&nbsp; <span style='color:#0d9488;'>Pure Regime Entry</span>
    &nbsp;·&nbsp; BTC Signals · ETHA Execution
  </div>
  <div style='background:#ccfbf1; border-radius:8px; padding:10px 14px;
    ...
    🔁 <b>Core idea:</b> Use Bitcoin's trend signatures (U1, D2, D3, V-Gate) as the signal
    engine, but buy and sell <b>ETHA</b> (iShares Ethereum Trust) instead of BTC. ETHA
    holds spot ETH — a high-beta crypto sibling steered by Bitcoin's cleaner regime
    read — with ETF liquidity and tax treatment as a security.
  </div>
  ...
  <div style='margin-bottom:12px;'>
    <div style='font-size:11px; font-weight:700; color:#115e59; text-transform:uppercase;
         letter-spacing:0.8px; margin-bottom:6px;'>📥 Entry — BTC signals trigger ETHA buy</div>
    <table style='width:100%; border-collapse:collapse; font-size:12px; color:#134e4a;'>
      <tr>
        <td style='width:36px; vertical-align:top; padding:3px 6px 3px 0; font-weight:700;
             color:#0d9488;'>①</td>
  ...
  </div>""" + _etha_footer, unsafe_allow_html=True)
    elif strategy_variant == "above_ma30":
        st.markdown("""
<div style='background:#f0fdfa; border:2px solid #0d9488; border-radius:12px;
     padding:16px 20px; margin:4px 0 16px 0; font-family:sans-serif;'>
  <div style='font-size:14px; font-weight:700; color:#134e4a; margin-bottom:14px;
       letter-spacing:0.3px;'>
    📊 TF2 + V-Gate on ETHA &nbsp;—&nbsp; BTC Signals · ETHA Execution
  </div>
  <div style='background:#ccfbf1; border-radius:8px; padding:10px 14px;
       margin-bottom:12px; font-size:12px; color:#134e4a; font-weight:600;'>
    🔁 <b>Core idea:</b> Use Bitcoin's trend signatures (U1, D2, D3, V-Gate) as the signal
...
<div style='background:#f0fdfa; border:2px solid #0d9488; border-radius:12px;
  ...
  <div style='font-size:14px; font-weight:700; color:#134e4a; margin-bottom:14px;
       letter-spacing:0.3px;'>
    📊 TF2 + V-Gate on ETHA &nbsp;—&nbsp; <span style='color:#0d9488;'>Confirmed Uptrend Entry</span>
    &nbsp;·&nbsp; BTC Signals · ETHA Execution
  </div>
  <div style='background:#ccfbf1; border-radius:8px; padding:10px 14px;
       margin-bottom:12px; font-size:12px; color:#134e4a; font-weight:600;'>
  ...
  <div style='margin-bottom:12px;'>
    ...
    if bt_full_oos is not None:
        ...
        if len(_onav_raw) > 1:
            ...
            bt_oos = dict(
                ...
                **{
                    _pk: bt_full_oos[_pk][bt_full_oos[_pk].index >= OOS_START]
                    for _pk in ("btc_price_series", "etha_price_series")
                    if bt_full_oos.get(_pk) is not None
                },
            )
    ...
    if s_full:
        lbl_full = (f"🌐 Full Market (inception → May 2026) ⚠️ Mixed IS/OOS<br>"
                    f"<span style='font-size:10px; font-weight:400; opacity:0.85;'>"
                    f"{pd.Timestamp(s_full['start_date']).strftime('%b %d, %Y')} → "
                    f"{pd.Timestamp(s_full['end_date']).strftime('%b %d, %Y')}</span>")
    else:
        lbl_full = "🌐 Full Market (inception → May 2026)"

    _sub4 = ("<th style='padding:5px 8px; text-align:center;'>📊 TF2+V-Gate (ETHA)</th>"
             "<th style='padding:5px 8px; text-align:center;'>🧾 TF2+V-Gate (35% STCG)</th>"
             "<th style='padding:5px 8px; text-align:center;'>🏦 B&amp;H ETHA (0% unrealised)</th>"
             "<th style='padding:5px 8px; text-align:center;'>💼 B&amp;H ETHA (15% LTCG)</th>")
    sub_hdr = (
        "<tr style='background:#334155; color:white; font-size:11px;'>"
        "<th style='padding:5px 12px;'></th>"
        + _sub4
    ...
    if bt_bear and bt_bear["open_pos"] and bt_bear["open_entry"]:
        ...
        st.markdown(
            ...
            f"📍 <b>Open ETHA position</b> — bought "
            f"{pd.Timestamp(oe['date']).strftime('%b %d, %Y')} "
            f"@ ${oe['price']:,.2f} · "
            f"unrealized: <span style='color:{oe_col}; font-weight:700;'>"
            f"{unr:+.1f}%</span>{_sl_str}</div>",
    ...
    def _make_etha_chart(bt, title):
        ...
        fig.add_trace(go.Scatter(
            x=bh_s.index, y=bh_s.values, name="Buy & Hold ETHA",
            line=dict(color="#94a3b8", width=1.5, dash="dot"),
            hovertemplate="%{x|%b %d, %Y}: $%{y:,.0f}<extra>Buy & Hold ETHA</extra>",
        ))
        fig.add_trace(go.Scatter(
            x=nav_s.index, y=nav_s.values, name="TF2+V-Gate (ETHA)",
            line=dict(color="#0d9488", width=2.5),
            hovertemplate="%{x|%b %d, %Y}: $%{y:,.0f}<extra>TF2+V-Gate (ETHA)</extra>",
        ))
        btc_price = bt.get("btc_price_series")
        if btc_price is not None and len(btc_price) > 0:
            fig.add_trace(go.Scatter(
        ...
        etha_price = bt.get("etha_price_series")
        if etha_price is not None and len(etha_price) > 0:
            fig.add_trace(go.Scatter(
                x=etha_price.index, y=etha_price.values, name="ETHA Price",
                line=dict(color="#6366f1", width=1.2, dash="dot"), yaxis="y3",
                opacity=0.8,
                hovertemplate="%{x|%b %d, %Y}: $%{y:,.2f}<extra>ETHA Price</extra>",
            ))
        fig.add_hline(
            y=s["initial_capital"], line_dash="dash",
            line_color="#64748b", line_width=1, opacity=0.4,
        ...
        for t in bt["trades"]:
            ...
            fig.add_trace(go.Scatter(
                ...
                showlegend=False,
                hovertemplate=(
                    f"<b>BUY ETHA</b> {pd.Timestamp(t['entry_date']).strftime('%b %d')}"
                    f" @ ${t['entry_price']:,.2f}<br>{t['entry_trigger']}<extra></extra>"
                ),
            ))
            fig.add_trace(go.Scatter(
                ...
                showlegend=False,
                hovertemplate=(
                    f"<b>SELL ETHA</b> {pd.Timestamp(t['exit_date']).strftime('%b %d')}"
                    f" @ ${t['exit_price']:,.2f} "
                    f"({t['pnl_pct']:+.1f}%) — {t['exit_signal']}<extra></extra>"
                ),
            ))
        if has_p and bt["open_entry"]:
            ...
            fig.add_trace(go.Scatter(
                ...
                showlegend=False,
                hovertemplate=(
                    f"<b>BUY ETHA (OPEN)</b> {pd.Timestamp(oe['date']).strftime('%b %d')}"
                    f" @ ${oe['price']:,.2f}<extra></extra>"
                ),
            ))
        fig.update_layout(
            ...
            yaxis3=dict(
                title="ETHA Price ($)", overlaying="y", side="right",
                showgrid=False, zeroline=False, anchor="free", position=1.0,
    ...
    if tab_labels:
        ...
        if bt_bear:
            with tabs[tab_idx]:
                sr = bt_bear["stats"]
                fig_r = _make_etha_chart(
                    bt_bear,
                    f"TF2+V-Gate (ETHA) vs B&H ETHA — Bear Market  "
                    f"({pd.Timestamp(sr['start_date']).strftime('%b %d, %Y')} → "
                    f"{pd.Timestamp(sr['end_date']).strftime('%b %d, %Y')})",
                )
                if fig_r:
                    st.plotly_chart(fig_r, use_container_width=True,
                                    key=f"etha_chart_bear_{key_suffix}")
            tab_idx += 1
        if bt_bull:
            with tabs[tab_idx]:
                sf = bt_bull["stats"]
                fig_f = _make_etha_chart(
                    bt_bull,
                    f"TF2+V-Gate (ETHA) vs B&H ETHA — Bull Market  "
                    f"({pd.Timestamp(sf['start_date']).strftime('%b %d, %Y')} → "
                    f"{pd.Timestamp(sf['end_date']).strftime('%b %d, %Y')})",
                )
                if fig_f:
                    st.plotly_chart(fig_f, use_container_width=True,
                                    key=f"etha_chart_bull_{key_suffix}")
            tab_idx += 1
        if bt_oos:
            with tabs[tab_idx]:
                so = bt_oos["stats"]
                fig_o = _make_etha_chart(
                    bt_oos,
                    f"TF2+V-Gate (ETHA) vs B&H ETHA — OOS Period (Fully Blind)  "
                    f"({pd.Timestamp(so['start_date']).strftime('%b %d, %Y')} → "
                    f"{pd.Timestamp(so['end_date']).strftime('%b %d, %Y')})",
                )
                if fig_o:
                    st.plotly_chart(fig_o, use_container_width=True,
                                    key=f"etha_chart_oos_{key_suffix}")
            tab_idx += 1
        if bt_full:
            with tabs[tab_idx]:
                sfl = bt_full["stats"]
                fig_fl = _make_etha_chart(
                    bt_full,
                    f"TF2+V-Gate (ETHA) vs B&H ETHA — Full Market  "
                    f"({pd.Timestamp(sfl['start_date']).strftime('%b %d, %Y')} → "
                    f"{pd.Timestamp(sfl['end_date']).strftime('%b %d, %Y')})",
                )
                if fig_fl:
                    st.plotly_chart(fig_fl, use_container_width=True,
                                    key=f"etha_chart_full_{key_suffix}")

    # ── Trade log ─────────────────────────────────────────────────────────────
    def _etha_trade_log_rows(bt):
        if bt is None:
            return [], False, None
        s     = bt["stats"]
        has_p = bt["open_pos"]
        ...
        for i, t in enumerate(bt["trades"], 1):
            ...
            rows.append({
                "#":           i,
                "Entry":       pd.Timestamp(t["entry_date"]).strftime("%b %d, %Y"),
                "Buy ETHA @":  f"${t['entry_price']:,.2f}",
                "BTC Trigger": t["entry_trigger"],
                "Exit":        pd.Timestamp(t["exit_date"]).strftime("%b %d, %Y"),
                "Sell ETHA @": f"${t['exit_price']:,.2f}",
                "P&L":         f"{t['pnl_pct']:+.1f}%",
                "Result":      result,
                "Days":        t["duration_days"],
                "BTC Signal":  t["exit_signal"],
    ...
    if log_tabs:
        ...
        for bt_src, lbl in [(bt_bear, "bear"), (bt_bull, "bull"),
            ...
            if bt_src is None:
                continue
            rows, has_p, s = _etha_trade_log_rows(bt_src)
            with ltabs[lt_idx]:
                if rows:
                    st.dataframe(pd.DataFrame(rows), hide_index=True,
                                 use_container_width=True)
                    ...
                    st.info("No trades in this period — strategy was in cash throughout.")
                caption = (
                    "💡 P&L at ETHA execution prices (same-day fill on the BTC signal bar). "
                    "Entry/exit triggered by BTC TF2+V-Gate. ETHA prices from the versioned dataset. "
                )
                if lbl == "oos":
                    caption += "✅ Fully OOS — BTC CT model never saw this data."
                elif lbl == "full":
  ...
  <div style='margin-bottom:12px;'>
    ...
    <table style='width:100%; border-collapse:collapse; font-size:12px; color:#134e4a;'>
      <tr>
        ...
        <td style='vertical-align:top; padding:3px 0;'>
          Fixed <b>−6%</b> from entry price — triggers on daily close below stop level.
          Widened from −3% (2026-07e): on a 2× fund the old −3% sat at only ~0.3σ and
          whipsawed out on routine noise. The vol-matched −6% (≈2× MSTR's old 3%) lifts
          <b>Full +309% → +524%</b> (Sharpe 1.08 → 1.27, MDD −48% → −42%, win-rate 33% → 62%).
        </td>
      </tr>
      <tr><td colspan='2' style='padding:4px 0;'></td></tr>
      <tr>
      ...
      <tr>
        ...
        <td style='vertical-align:top; padding:3px 0;'>
          Within <b>12 bars</b> of a −6% stop, a fresh <b>U1 above MA30</b> re-admits even if the
          XOR combined-block would veto. Paired with a <b>5-bar V-reversal window</b> (2026-07c),
          MSTU reliably enters post-capitulation rallies (e.g. mid-Apr→mid-May 2025) across data
          vintages. With the wider −6% stop the sleeve is stopped out far less often (fewer, deeper
          stops instead of noise whipsaws), so the override fires rarely; <b>MSTU Full +524%</b>.
        </td>
      </tr>
    </table>
  </div>
...
def render_trend_signatures(sigs: dict, *, intraday: dict = None, open_positions: dict = None):
    ...
    Organized as:
      ...
      • Standard MA full-width card (live strategy — BTC/MSTR/MSTU/ETHA)
      • V-reversal special signal
    ...
    ALERT_CFG = {
        ...
        "STRATEGY_BUY":{"bg": "#eff6ff", "border": "#2563eb", "badge_bg": "#2563eb",
                        "badge_txt": "📊 STANDARD MA BUY",  "txt_col": "#1e3a8a"},
        ...
        <div style="
            ...
            <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                ...
                <span style="color:{cfg['txt_col']}; font-size:13px;">
                    ...
                    Entry (Standard MA): <b>{'✅ ACTIVE' if sigs['tf1_triggered'] else '○ inactive'}</b>
                    ...
                    <span style="color:#64748b;">· MSTR/MSTU/ETHA entries shown below (asset-specific gates)</span>
                    · as-of: <b>{as_of_str}</b>
    ...
    # ── ACTION SIGNAL banners — Row 1: BTC  /  Row 2: MSTR, MSTU & ETHA ─────
    ...
    _entry_mstr   = _entry_signal                                    # MSTR/MSTU/ETHA — same Standard MA gate
    _entry_mstu   = _entry_signal
    _entry_etha   = _entry_signal
    ...
    if _exit_signal and _entry_signal:
        ...
        _entry_gates = []
        if _above_ma30 and not _clean_7d:             _entry_gates.append("📈Above MA30")
        if _clean_7d and not _above_ma30:             _entry_gates.append("🧹Clean Breakout")
        ...
        _btc_both_active = _above_ma30 and _clean_7d and not _v_gate_ok
        _btc_label = "U1 ACTIVE — TREND GATE BLOCKED"
        _btc_sub   = (
            (
                "U1 fired but entry blocked: Above MA30 AND Clean 7d both active — the XOR gate "
                "treats that as an ambiguous late-cycle entry. Wait for one side to resolve "
                "(a D1/D2 to reset the Clean 7d window), or a ⚡V-reversal."
                if _btc_both_active else
                "U1 fired but no trend path: the Standard MA gate needs exactly one of Above MA30 "
                "or Clean 7d (not both, not neither), or a ⚡V-reversal."
            )
            + f"  Above MA30={'YES' if _above_ma30 else 'NO'}, "
            + f"Clean 7d={'YES' if _clean_7d else 'NO'}, "
    ...
    st.markdown(
        ...
        <div style="background:{_btc_bg}; border:2.5px solid {_btc_brd};
          ...
          <div style="margin-left:auto; font-size:11px; color:#64748b;
            ...
            <b>Standard MA Strategy</b><br>Signal as of today's close
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # ── Row 2: MSTR / MSTU / ETHA with SL5 cooldown awareness ────────────
    if open_positions:
        def _sl5_chip(pos_data: dict, entry_ok: bool, gate_blocked: bool):
            ...
            entry_ok     = the asset's entry gate (Standard MA for MSTR/MSTU/ETHA) fires today.
        ...
        # Each asset evaluated against ITS OWN entry gate.
        _mstr_lbl, _mstr_note, _mstr_bg, _mstr_brd, _mstr_col = _sl5_chip(
        ...
        _etha_lbl, _etha_note, _etha_bg, _etha_brd, _etha_col = _sl5_chip(
            open_positions.get("etha", {}), _entry_etha, sigs["u1_triggered"] and not _entry_etha)

        # Overall panel border: driven by the most urgent state across the assets
        _both = {_mstr_lbl, _mstu_lbl, _etha_lbl}
        ...
        # 2026-07 retune: BTC, MSTR & MSTU all trade the Standard MA (above-MA30) entry
        # gate; the regime-adaptive D2/D3 exit is shared. Only the fixed stop differs
        # (2026-07e: MSTU −6%, MSTR & BTC none / signal-exit-only).
        ...
        _eq_sub = (
            ...
            f"(same signal as BTC; MSTU −6% stop, MSTR & ETHA signal-exit-only)"
            f" &nbsp;|&nbsp; Exit (regime-adaptive D2/D3): {_exit_txt}"
            f" &nbsp;|&nbsp; Regime: {_regime_label}"
        )

        def _chip_html(label, note, bg, brd, col, asset_name, gate_name):
            inner = (
                f"<b>{asset_name}</b> <span style='font-size:10px;font-weight:600;color:#64748b;'>({gate_name})</span>"
                f" &nbsp; {label}"
        ...
        st.markdown(
            ...
            <div style="background:{_eq_bg}; border:2.5px solid {_eq_brd};
              ...
              <div style="font-size:10px; font-weight:700; color:#64748b;
                ...
                📈 MSTR · MSTU · ETHA (📊 Standard MA — MSTR &amp; ETHA signal-exit-only · MSTU fixed −6% + SL5 re-entry) — shared regime-adaptive D2/D3 exit
              </div>
              <div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:7px; align-items:flex-start;">
                ...
                {_chip_html(_etha_lbl, _etha_note, _etha_bg, _etha_brd, _etha_col, "ETHA", "Standard MA")}
              </div>
    ...
    if open_positions:
        ...
        _SIG_CFG = {
            ...
            "SL-fixed-6%":  ("🛑", "Stop Loss hit — Fixed −6%",   "#fef2f2", "#dc2626", "#991b1b"),
        ...
        for _asset_key in ("btc", "mstr", "mstu", "etha"):
    ...
    # ── Standard MA full-width card (U1 + Standard MA gate + V-reversal + regime exit) ─
    # Rows mirror the "Strategy Rules at a Glance" box exactly: the entry trigger is
    #   tf1_triggered = U1 AND ((above_MA30 XOR Clean 7d) OR V-reversal)
    # The Standard MA gate has two non-overlapping trend paths: 📈 Above MA30 (momentum
    # leg — BTC above its 30-day MA) and 🧹 Clean Breakout (Clean 7d while below MA30 — a
    # fresh thrust off a bottom). The XOR blocks the ambiguous both-true / neither-true
    # states. Regime (Bull = BTC > MA30 AND MA30 rising) still drives the exit mode only.
    ...
    _bull   = sigs.get("bull_regime", False)        # _above AND _rising — exit mode only
    _clean  = sigs["clean_10d"]
    _vrev   = sigs.get("v_recent_gate", False)
    _above_only     = _above and not _clean         # momentum leg (above MA30, not a washout)
    _clean_breakout = _clean and not _above         # washed-out Clean thrust (below MA30)
    _gate_pass = _above_only or _clean_breakout or _vrev  # Standard MA gate: (above XOR clean) OR V-rev

    # 📈 Above MA30 status text
    if _above_only:
        _above_disp = "YES — BTC > MA30 (momentum leg, not a washout)"
    elif _above and _clean:
        _above_disp = "NO — BTC > MA30 but also Clean 7d (XOR blocks both-true)"
    else:
        _above_disp = "NO — BTC ≤ MA30"

    # Standard MA gate status text
    if _above_only:
        _gate_disp = "PASS ✅ — 📈 Above MA30 (BTC > MA30, momentum leg)"
    elif _clean_breakout:
        _gate_disp = "PASS ✅ — 🧹 Clean Breakout (Clean 7d while below MA30)"
    elif _vrev:
        _gate_disp = "PASS ✅ — ⚡ V-reversal override"
    elif _above and _clean:
        _gate_disp = "✗ — Above MA30 AND Clean 7d both true (XOR blocks — ambiguous)"
    else:
        _gate_disp = "✗ — neither Above MA30 nor Clean 7d (no trend path)"
    ...
    tf1_rows = [
        ...
        ("① U1 Signal",
         ...
         _u1, True),
        # ── Entry condition ② — Standard MA gate (Above MA30 XOR Clean Breakout, or V-rev) ──
        ("② 📈 Above MA30",
         _above_disp,
         "= YES  (gate path A — BTC > MA30 AND NOT Clean 7d)",
         _above_only, True),
        ("   ↳ BTC vs 30-day MA",
         f"${sigs['current_close_sig']:,.0f}  vs  MA=${sigs['ma30_value']:,.0f} ({_ma30_pct:+.1f}%)",
         "> MA30  (Above-MA30 sub-condition)",
         _above, False),
        ("   ↳ MA30 slope (5-bar)",
         f"MA30 = ${sigs['ma30_value']:,.0f}  vs  5d ago = "
         f"${sigs.get('ma30_5d_ago', sigs['ma30_value']):,.0f} ({_slope_pct:+.2f}%)",
         "rising  (regime context — drives exit mode)",
         _rising, False),
        ("② 🧹 Clean Breakout",
         ("YES — Clean 7d (no D1/D2) AND BTC below MA30 (fresh thrust off a washout)"
          if _clean_breakout else
          ("NO — Clean 7d but BTC already above MA30 (that is the Above-MA30 leg, XOR blocks both)"
        ...
        ("② Standard MA gate",
         _gate_disp,
         "= PASS on 📈 Above MA30 XOR 🧹 Clean Breakout (exactly one), OR ⚡ V-reversal",
         _gate_pass, True),
        ...
        ("Fixed stop (per asset)",
         "MSTU −6% · MSTR none · BTC none — close-triggered, filled at close",
         "MSTU exits if daily close ≤ entry × 0.94; MSTR & BTC use D2/D3 signal exits only",
         False, False),
        ("Re-entry after stop (SL5)",
         "MSTU only — BULL regime → re-enter next valid signal · BEAR/Neutral → wait 10 bars",
         "regime-adaptive cooldown avoids dead-cat re-entries (MSTU is the only stopped sleeve)",
         False, False),
        ("Post-stop re-entry override",
         "MSTU: within 12 bars of a −6% stop, a fresh U1 above MA30 re-admits (bypasses XOR block)",
         "recovers post-capitulation rallies the stop+XOR block used to lock MSTU out of (e.g. Apr 2025); MSTR & BTC (no stop) unaffected",
         False, False),
    ]
    st.markdown(
        _sig_card(
            title="📊 Standard MA Strategy (BTC · MSTR · MSTU · ETHA) — U1 + (Above MA30 XOR Clean Breakout) / V-reversal · Regime-Adaptive Exit",
            icon="📊",
            color="#2563eb",
            triggered=sigs["tf1_triggered"],
            signal_rows=tf1_rows,
            interpretation=(
                "<b>All four assets — BTC, MSTR, MSTU and ETHA — trade the same Standard MA gate</b> "
                ...
                "📈 <b>Above MA30</b> (BTC above its 30-day MA — momentum leg) <b>XOR</b> "
                "🧹 <b>Clean Breakout</b> (Clean 7d while BTC is below MA30 — a fresh thrust off a washout) "
                "— <b>or</b> a ⚡ <b>V-reversal</b> capitulation within 3 bars. The XOR blocks the ambiguous "
                ...
                "MSTR, BTC and ETHA are signal-exit-only (no fixed stop)."
            ),
            timing=(
            ...
            prob_txt=(
                "Full period (Jun 2024–May 2026), Standard MA, 2026-07e stops (MSTR none · MSTU −6%) "
                "+ post-stop re-entry override: "
                ...
                "ETHA (added 2026-07f, from its Jul-2024 inception) is reported on its own "
                "🔹 ETHA Backtesting tab — the CT engine turns its deeply negative buy-&amp;-hold "
            ...
            conf_txt=(
                ...
                "results ride ~8 trades/period and (for MSTU) ~2 large rally trades. "
                "Bear period is partly OOS. MSTU's −6% stop is vol-matched to its ~2× volatility "
                "(was −3%); MSTR now runs stop-free. "
...
- 📊 **STANDARD MA BUY**: U1 + Standard MA gate ((Above MA30 **XOR** washed-out Clean Breakout) OR ⚡V-reversal) → entry (see Standard MA card below)
...
**Regime-Adaptive Strategy — unified Standard MA entry (2026-07 retune):**
...
- **Stops (per-asset · 2026-07e/f):** MSTU −6% (vol-matched to its ~2× vol; was −3%) with SL5 regime-adaptive re-entry + post-stop re-entry override · MSTR, BTC & ETHA signal-exit-only (no fixed stop)
...
- **ETHA** (added 2026-07f, 1× spot-ETH ETF traded on the same gate from its Jul-2024 inception): see the 🔹 **ETHA Backtesting** tab for its own four-period results — the CT signal turns ETHA's deeply negative buy-&-hold into a strong positive at roughly a quarter of the drawdown
...
def render_dashboard(as_of_t, *, is_live, live_spot=None, live_spot_ts=None,
    ...
    # ─────────── MSTR / MSTU / ETHA prices + 743d ATM call ──────────────
    ...
    if is_live:
        ...
        _disp_etha_px,  _disp_etha_chg = _csv_price_and_chg(_load_etha_prices())
    ...
    e3.metric(
        _etha_lbl,
        f"${_disp_etha_px:,.2f}" if _disp_etha_px is not None else "—",
        delta=(f"{_disp_etha_chg:+.2f}% {_etha_delta_sfx}" if _disp_etha_chg is not None else None),
    )
    e4.metric(
        _strc_lbl,
    ...
    if sigs is not None:
        ...
        # Compute MSTR/MSTU/ETHA OOS positions for the live position tracker.
        ...
        # position) even though the shared entry signal had opened a position,
        ...
        # ETHA starts at its 2024-07-23 inception (run_etha_backtest clamps internally).
        _bt_etha_oos = run_etha_backtest(_bt_oos_end, backtest_start_iso="2024-06-01", model_mtime=_model_mtime, data_end=_data_end or "", data_mtime=_ds_mtime, entry_gate=ETHA_STRATEGY_GATE)
        ...
        if is_live:
            ...
            _etha_panel_px = _bt_mark(_bt_etha_oos, "etha_price_series")
        ...
        _open_positions: dict = {
            ...
            "mstr": dict(
                ...
                asset_label   = "MSTR",
                stop_type     = (f"fixed-{int(round(MSTR_STOP_PCT*100))}%" if MSTR_STOP_PCT is not None else "none"),
            ...
            "etha": dict(
                open_pos      = bool(_bt_etha_oos and _bt_etha_oos.get("open_pos")),
                entry         = (_bt_etha_oos or {}).get("open_entry"),
                live_price    = _etha_panel_px,
                asset_label   = "ETHA",
                stop_type     = (f"fixed-{int(round(ETHA_STOP_PCT*100))}%" if ETHA_STOP_PCT is not None else "none"),
                nav           = (_bt_etha_oos or {}).get("stats", {}).get("final_nav"),
                last_trade    = _last_closed_trade(_bt_etha_oos),
                from_sl       = (_bt_etha_oos or {}).get("from_sl", False),
                bars_since_sl = (_bt_etha_oos or {}).get("bars_since_sl", 0),
            ),
        }
    ...
    fig.update_layout(
        ...
        yaxis_title="BTC / USD",
        margin=dict(t=115, r=40, b=130, l=70),
    ...
    if len(series) > 0:
        st.markdown(
            ...
            f"🔻 D1/D2/D3 + 🔼 U1 signature markers; D1/D2 set the Clean-Breakout window)"
        )
        fig2 = go.Figure()
        ...
        # 🔻 D1 / D2 / D3 + 🔼 U1 signature markers over the Clean-Breakout
        # lookback window.  sigs carries per-bar d1_hist / d2_hist / d3_hist /
        # u1_hist (aligned to sig_dates); d1/d2 are the exact arrays the gate
        # scans (clean_10d = no D1/D2 in the prior 7 bars).  D1 (downtrend /
        # low-break pressure) sits on the predicted-LOW line, D2 (high-momentum
        # fade) on the predicted-HIGH line, D3 (exhaustion canary) on the actual
        # LOW that broke the floor, U1 (bullish breakout pressure) on the actual
        # HIGH.  Marker positions are read from the plotted series so they land
        # exactly on the plotted lines/points.
        if sigs and sigs.get("d1_hist") is not None and sigs.get("sig_dates") is not None:
            ...
            _d3_by = (dict(zip(_sig_dts, [bool(x) for x in sigs["d3_hist"]]))
                      if sigs.get("d3_hist") is not None else {})
            _u1_by = (dict(zip(_sig_dts, [bool(x) for x in sigs["u1_hist"]]))
            ...
            for _, _r in series.iterrows():
                ...
                if _d3_by.get(_td):
                    _d3x.append(_td)
                    _d3y.append(_r["actual_low"] if pd.notna(_r["actual_low"])
                                else _r["pred_low"])
                if _u1_by.get(_td):
                    _u1x.append(_td)
                    _u1y.append(_r["actual_high"] if pd.notna(_r["actual_high"])
                                else _r["pred_high"])
            if _d1x:
                fig2.add_trace(go.Scatter(
            ...
            if _d3x:
                fig2.add_trace(go.Scatter(
                    x=_d3x, y=_d3y, mode="markers+text", text=["D3"] * len(_d3x),
                    textposition="bottom center", textfont=dict(size=11, color="#6d28d9"),
                    marker=dict(symbol="triangle-down", size=14, color="#7c3aed",
                                line=dict(width=1.4, color="white")),
                    name="D3 fired — exhaustion canary",
                    hovertemplate=("Bar %{x|%Y-%m-%d}: 🔻 D3 — exhaustion canary "
                                   "(first low-break after ≥3 high-break "
                                   "streak)<extra></extra>")))
            if _u1x:
                fig2.add_trace(go.Scatter(
                    x=_u1x, y=_u1y, mode="markers+text", text=["U1"] * len(_u1x),
                    textposition="top center", textfont=dict(size=11, color="#15803d"),
                    marker=dict(symbol="triangle-up", size=14, color="#16a34a",
                                line=dict(width=1.4, color="white")),
                    name="U1 fired — bullish breakout pressure",
                    hovertemplate=("Bar %{x|%Y-%m-%d}: 🔼 U1 — bullish pressure "
                                   "(3d-avg high overshoot + ≥2 "
                                   "high-breaks)<extra></extra>")))
        ...
        fig2.update_layout(
            ...
            margin=dict(t=140, r=40, b=120, l=70),
...
# ════════════════════════════════════════════════════════════════════════
tab_live, tab_hist, tab_btc, tab_mstr, tab_mstu, tab_etha, tab_mstr_opts, tab_mstu_opts, tab_explain, tab_leading, tab_retrain = st.tabs([
    "🔴 Live (rolling now+1h)",
    "🕒 Historical replay",
    "₿ BTC Backtesting",
    "📊 MSTR Backtesting",
...
with tab_btc:
    ...
    st.markdown(
        ...
        "**⭐ Default strategy: 📊 Standard MA** entry gate (above rising MA30) — delivers "
        "**Full +88%** (vs B&H +6%), Bull +47%, and stays positive in bear (+13% vs B&H −30%). "
        "No stop-loss — positions close only on D2 or D3 regime exit signals. Switch gates with the radio below."
    )
    ...
    st.info(
        ...
        "bypassing the XOR gate) **does not apply here**. After the 2026-07e retune it exists "
        "only on the **MSTU** sleeve (the sole stopped sleeve — MSTR is now signal-exit-only), "
        "where it recovers post-capitulation rallies (e.g. mid-Apr→mid-May 2025). BTC exits are "
        "managed entirely by the D2/D3 regime rules."
    )
    _btc_variant = st.radio(
        "Entry gate variant",
        options=["above_ma30", "pure_regime", "bull_regime", "pure_regime_sata"],
        index=0,   # ⭐ BTC default = Standard MA (2026-07 retune; unified across BTC/MSTR/MSTU)
        format_func=lambda x: (
            "📊 Standard MA — Above rising MA30  ⭐ STRATEGY"
...
with tab_mstr:
    ...
    st.markdown(
        ...
        "**Full +266%** (vs B&H +4%), Bull +142%, and staying positive in bear (+57% vs "
        "B&H −57%), with strong Sharpe (~1.4) and −22% drawdown. "
        "**2026-07e: MSTR now runs signal-exit-only — no fixed stop.** Its old −3% stop only "
        "whipsawed the sleeve out during bull pullbacks (a 1× name has no wipeout tail), so "
        "removing it lifts Full +212% → +266% (Sharpe 1.34 → 1.41, win-rate 75% → 85%, drawdown "
        "unchanged). Exit is the shared regime-adaptive D2/D3 rule; entries still use the "
        "**5-bar V-reversal window** so the capitulation→U1 bridge reliably captures rallies like "
        "mid-Apr→mid-May 2025 across data vintages. With no stop there is no SL re-entry or "
        "post-stop override. Switch gates with the radio below."
    )
    ...
    st.info(
        "🔒 **MSTR is signal-exit-only (2026-07e):** MSTR carries **no fixed stop** — positions "
        "close only on the shared D2/D3 regime exits. Its old −3% stop only whipsawed the sleeve "
        "out during bull pullbacks, so removing it lifts **Full +212% → +266%** (Sharpe 1.34 → 1.41, "
        "win-rate 75% → 85%, drawdown unchanged −22%). With no stop there is no SL cooldown or "
        "post-stop override. Entries still use the **5-bar V-reversal window** (2026-07c) so the "
        "capitulation→U1 bridge reliably captures rallies like mid-Apr→mid-May 2025 across data "
        "vintages (on the narrower 3-bar window the live vintage missed the rally entirely)."
    )
    _mstr_variant = st.radio(
        "Entry gate variant",
        options=["above_ma30", "pure_regime", "bull_regime", "pure_regime_sata"],
        index=0,   # ⭐ MSTR default = Standard MA (re-tuned 2026-07; most profitable + stable)
        format_func=lambda x: (
            "📊 Standard MA — Above rising MA30  ⭐ MSTR STRATEGY"
            if x == "above_ma30" else
            "🎯 Pure Regime — Bull Regime or Clean Breakout"
            if x == "pure_regime" else
            "🔒 Confirmed Uptrend — Bull Regime (XOR)"
...
with tab_mstu:
    ...
    st.markdown(
        ...
        "with a **fixed −6% stop** — the Standard MA gate delivers **Full +524%** (vs B&H −76%), "
        "Bull +229%, and keeps Bear strongly positive (+101% vs B&H −92%) by avoiding the "
        "leveraged-ETF volatility decay B&H suffers. **2026-07e: the stop was widened −3% → −6%** "
        "(vol-matched to MSTU's ~2× volatility — the old −3% sat at only ~0.3σ and whipsawed out "
        "on routine noise), lifting Full +309% → +524% (Sharpe 1.08 → 1.27, MDD −48% → −42%, "
        "win-rate 33% → 62%) while still truncating the crash tail that going stopless opens up. "
        "Exit is the shared regime-adaptive D2/D3 rule with SL5 re-entry **plus the post-stop "
        "re-entry override** (fresh U1 above the MA30 within 12 bars of a stop) and a **5-bar "
        "V-reversal window** so MSTU reliably enters rallies like mid-Apr→mid-May 2025 across data "
        "vintages. MSTU launched Sep 18 2024; the Bull period uses synthetic MSTU prices (OLS from "
        "MSTR). Switch gates with the radio below."
    )
    ...
    st.info(
        "🛑 **MSTU stop retune (2026-07e): −3% → −6%.** On a 2× fund the old −3% stop sat at only "
        "~0.3σ and stopped out on routine noise, whipsawing the sleeve. The **vol-matched −6%** "
        "(≈2× MSTR's old 3%) fixes this: **Full +309% → +524%** (Sharpe 1.08 → 1.27, MDD −48% → "
        "−42%, win-rate 33% → 62%), while still truncating the catastrophic crash tail that going "
        "fully stopless opens up (P(MDD<−80%) climbs toward ~44% stopless vs ~26% at 6–8%). The "
        "**post-stop re-entry override** (fresh U1 above MA30 within 12 bars of a stop) plus the "
        "**5-bar V-reversal window** still capture post-capitulation rallies; with the wider stop "
        "MSTU is stopped out far less often, so the override fires rarely. On top of SL5 "
        "regime-adaptive re-entry (BULL: next signal · BEAR/Neutral: wait 10 bars)."
    )
    _mstu_variant = st.radio(
        "Entry gate variant",
        options=["above_ma30", "pure_regime", "bull_regime", "pure_regime_sata"],
        index=0,   # ⭐ MSTU default = Standard MA (2026-07e: −6% stop; Full +524% w/ vintage-robust rally capture)
        format_func=lambda x: (
...
with tab_etha:
    st.markdown("## 🔹 ETHA — BTC Signal-Driven Backtesting")
    st.markdown(
        "Trades in **ETHA (iShares Ethereum Trust — 1× spot-ETH ETF)**, driven by BTC "
        "CT-model signals. **⭐ Default strategy: 📊 Standard MA** entry gate (above rising "
        "MA30) with **no fixed stop** — the exact MSTR treatment, applied to the fourth "
        "sleeve on the shared BTC signal (2026-07f add). ETH trades as a high-beta crypto "
        "sibling of Bitcoin, so BTC's cleaner regime read steers the ETH sleeve: the "
        "long/flat signal turns ETHA's deeply negative buy-&-hold (the fund round-tripped "
        "its 2025 run-up) into a strong positive at a fraction of the drawdown, and every "
        "fixed stop ≤8% tested only reduced return and Sharpe. Full validation — including "
        "the rejected own-price and BMNR alternatives — is in **ETHA_BMNR_STRATEGY_EVAL.md**. "
        "ETHA listed Jul 23 2024; all backtest windows are clamped to inception (no synthetic "
        "pre-listing history). Switch gates with the radio below."
    )
    _ds_ver_etha = _backtest_dataset_version()
    _ds_mtime_etha = _backtest_dataset_mtime()
    st.caption(f"📦 Price dataset {_ds_ver_etha} · pulled via `scripts/pull_backtest_data.py` · all QC checks passed")
    st.info(
        "🔒 **ETHA is signal-exit-only (2026-07f):** positions close only on the shared "
        "regime-adaptive D2/D3 exits — no fixed stop, so there is no SL cooldown or post-stop "
        "override. Entries use the same U1 + Standard-MA gate and **5-bar V-reversal window** "
        "as MSTR/MSTU. Note the CT engine's numbers drift modestly with feature-data "
        "refreshes (on-chain/premium series are revised between pulls); the qualitative "
        "ranking — BTC-signal ≫ own-price rules ≫ buy-&-hold — has held on every vintage."
    )
    _etha_variant = st.radio(
        "Entry gate variant",
        options=["above_ma30", "pure_regime", "bull_regime", "pure_regime_sata"],
        index=0,   # ⭐ ETHA default = Standard MA (2026-07f add; MSTR treatment, no stop)
        format_func=lambda x: (
            "📊 Standard MA — Above rising MA30  ⭐ ETHA STRATEGY"
            if x == "above_ma30" else
            "🎯 Pure Regime — Bull Regime or Clean Breakout"
            if x == "pure_regime" else
            "🔒 Confirmed Uptrend — Bull Regime (XOR)"
        ...
        horizontal=True,
        key="etha_variant_radio",
    )
    if _etha_variant == "pure_regime_sata":
        st.caption(
            "🪙 **SATA idle-cash variant** — *identical* Pure Regime (Bull Regime OR Clean Breakout OR V-reversal) "
            "entry/exit signals; the **only** difference is that capital sitting in **cash "
            "between positions** is parked in **SATA** (Strive's Variable Rate Series A "
    ...
    _etha_model_mtime = (float(os.path.getmtime(str(DAILY_MODEL_CT)))
                         if os.path.exists(str(DAILY_MODEL_CT)) else 0.0)
    _etha_oos_end     = (pd.Timestamp.now(tz="UTC") - pd.Timedelta(days=1)).normalize().strftime("%Y-%m-%d")
    _etha_raw      = _fetch_daily_raw()
    _etha_data_end = _etha_raw.index.max().strftime("%Y-%m-%d") if not _etha_raw.empty else ""
    _etha_bear     = _run_fixed_period_etha_backtest(
        "2026-05-31", "2025-06-01", _etha_model_mtime, data_end=_etha_data_end,
        data_mtime=_ds_mtime_etha, entry_gate=_etha_variant)    # locked Jun 2025–May 2026
    _etha_bull     = _run_fixed_period_etha_backtest(
        "2025-05-31", "2024-06-01", _etha_model_mtime, data_end=_etha_data_end,
        data_mtime=_ds_mtime_etha, entry_gate=_etha_variant)    # Bull: clamped Jul 23 2024–May 31 2025
    _etha_full_oos = run_etha_backtest(
        _etha_oos_end, model_mtime=_etha_model_mtime, data_end=_etha_data_end,
        data_mtime=_ds_mtime_etha, entry_gate=_etha_variant)    # OOS ends prior day (rolling)
    _etha_full     = _run_fixed_period_etha_backtest(
        "2026-05-31", "2024-06-01", _etha_model_mtime, data_end=_etha_data_end,
        data_mtime=_ds_mtime_etha, entry_gate=_etha_variant)    # Full: clamped Jul 23 2024–May 2026
    render_etha_trading_strategy_dashboard(
        _etha_bear, _etha_bull,
        bt_full_oos=_etha_full_oos,
        bt_full=_etha_full,
        key_suffix="etha_tab",
        strategy_variant=_etha_variant,
    )
...
with tab_mstu_opts:
    ...
    st.markdown(
        ...
        "📊 Standard MA** — same entry gate as the MSTU stock tab. Exit is the shared "
    ...
    _mstu_opts_variant = st.radio(
        "Entry gate variant",
        options=["above_ma30", "pure_regime", "bull_regime"],
        index=0,   # ⭐ MSTU options default = Standard MA (matches MSTU stock strategy)
        format_func=lambda x: (
            "📊 Standard MA — Above rising MA30  ⭐ MSTU STRATEGY"
            if x == "above_ma30" else
            "🎯 Pure Regime — Bull Regime or Clean Breakout"
...
with tab_mstr_opts:
    ...
    st.markdown(
        "Executes in **MSTR ATM call options** (strike = MSTR spot at entry, 743d expiry, "
        "Black-Scholes priced, σ = 60-day rolling MSTR vol, r = 4.5%). **⭐ Default strategy: "
        "📊 Standard MA** — same entry gate as the MSTR stock tab. Exit is the shared "
        "regime-adaptive D2/D3 rule. B&H benchmark is MSTR stock (unleveraged)."
    )
    _opts_variant = st.radio(
        "Entry gate variant",
        options=["above_ma30", "pure_regime", "bull_regime"],
        index=0,   # ⭐ MSTR options default = Standard MA (matches MSTR stock strategy)
        format_func=lambda x: (
            "📊 Standard MA — Above rising MA30  ⭐ MSTR STRATEGY"
            if x == "above_ma30" else
            "🎯 Pure Regime — Bull Regime or Clean Breakout"
