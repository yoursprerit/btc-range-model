...
def _rsi(close: pd.Series, period: int = 14) -> pd.Series:
    ...
    (observed: SOXX rallied 14 straight sessions into 2026-04-20, dropping five
    April rows and sliding the trend engine's SMA windows five bars — see the
    trend-signal note in ``backtest_ticker.build_predictions``).  The mirror
    case — no up bar in the window — is RSI 0.
    """
    delta = close.diff()
