...
| `--catch-up` | **rescue a missed slot.** For the run that fires late because the laptop was off at 2:30 PM CT: inert while the session is open, and past the close it places the day's book as `outsideRth` limits out to 20:00 ET; past that (or on a weekend/holiday) it aborts, because no order placed then would fill and tomorrow the book is stale by construction. It skips any signal bar that already has an execution report, so it is safe to leave on permanently — **the wrapper passes it by default**; `setx IBKR_CATCH_UP 0` opts out. See IBKR_PAPER_TRADING.md § When the slot was missed. |
| `--refresh-report` | **place no orders** — connect, read the account's real positions and today's fills from IBKR, and rewrite a **signed** report. Use it when late or partial fills print after the sending run's `--fill-timeout` expired (routine outside RTH), or to re-sign a report written from a shell that had no `OVERALL_BOOK_SECRET`. **Same day as the trade only:** IBKR serves the current session's fills, so a refresh run a day late finds none — it now aborts rather than writing a trade-less report over the real one. To publish a report that was never pushed, commit `data/overall/executed_book.json` and `data/overall/executed_archive/` by hand. |
| `--push-report` / `--no-push-report` | commit + push the report. **ON by default** for a run that trades or refreshes — a report that never leaves the laptop is a silent failure: the trade happened and the cloud app goes on showing the last *scheduled* run's book. `--no-push-report` (or `IBKR_NO_PUSH_REPORT=1`) opts out on a host with no git write credentials. |
...
- **`-StartWhenAvailable` re-fires a missed run, but by itself it cannot trade.**
  ...
  that is after the 4:00 PM ET close, the session-hours guard would abort it and
  the day would go untraded. So the wrapper passes `--catch-up` **by default**,
  and that late fire still places the day's book in extended hours (to 20:00 ET).
  It changes nothing about an on-time fire, and it never re-trades a bar that
  already has an execution report; `setx IBKR_CATCH_UP 0` opts out.
...
| `ABORT: account already carries a margin loan` | The account is not where the last run left it. Flatten the loan (or reconcile manually), then re-run. |
| `buys $… exceed $… available — scaled to …%` | Not an error: the sell leg realised less than planned, so the buy leg was trimmed to the cash that funds it. Names dropped entirely show as `SKIPPED-FUNDING` in the Executed Book. The next session's run closes the gap. |
| `ABORT: … outside regular trading hours` | The run fired outside 09:30–16:00 ET. Check the scheduled time (§7); `--outside-rth` only for a deliberate manual run. |
