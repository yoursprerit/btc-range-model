"""Idea 1 Entry Signal Comparison — MSTR & MSTU

Compares the CURRENT entry gate:
    tf1_entry = u1 & ((above_ma30 ^ clean_10d) | v_recent)

vs IDEA 1 entry gate (replace above_ma30 with bull_regime):
    tf1_entry = u1 & ((bull_regime ^ clean_10d) | v_recent)

bull_regime = above_ma30 AND ma30_slope_pos (MA30 is also rising).
This blocks entries when price is above a DECLINING MA30 — typical of
bear bounces — while leaving all genuine bull-market entries intact.
...
def compute_signals(comp: pd.DataFrame, entry_mode: str):
    """Return dict of signal arrays. entry_mode: 'current' or 'idea1'."""
    N       = len(comp)
    ...
    if entry_mode == "idea1":
        # Idea 1: replace above_ma30 with bull_regime in the XOR gate
        tf1_entry = u1 & ((bull_regime ^ clean_10d) | v_recent)
    else:
        # Current / baseline
        tf1_entry = u1 & ((above_ma30 ^ clean_10d) | v_recent)
...
def main():
    print("\n" + "═"*70)
    print("  Idea 1 Comparison — MSTR & MSTU (all UI periods)")
    print("  Entry gate: bull_regime ^ clean_10d  vs  above_ma30 ^ clean_10d")
    print("═"*70)
    ...
    for asset_label, ticker, stop_pct, periods in configs:
        ...
        for period_name, (end_iso, start_iso) in periods.items():
            ...
            r_i1  = run_asset_backtest(preds_df, raw_df, ticker, start_iso, end_iso, stop_pct, "idea1")
            if r_cur and r_i1:
                delta_ret = r_i1["strat_ret"] - r_cur["strat_ret"]
                delta_dd  = r_i1["max_dd"]    - r_cur["max_dd"]
                delta_sh  = r_i1["sharpe"]    - r_cur["sharpe"]
                ...
                print(f"  {'':8}  {'Δ idea1':<9}"
                      ...
                      f"  {'':>6}"
                      f"  {r_i1['n_trades']-r_cur['n_trades']:>+4}"
                      f"  {r_i1['n_sl']-r_cur['n_sl']:>+4}")
            print()
