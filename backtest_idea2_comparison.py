"""Idea 2 Entry Signal Comparison — MSTR & MSTU

Compares the CURRENT entry gate where U1 fires whenever:
    ehma3 > 0.7%  AND  hb3 >= 2

vs IDEA 2 — Error momentum gate: require ehma3 to be RISING on the
entry bar (today's 3-day average > yesterday's 3-day average):
    ehma3_rising[i] = ehma3[i] > ehma3[i-1]
    u1 = (ehma3 > 0.7) & (hb3 >= 2) & ehma3_rising

A rising error average means breakout pressure is accelerating.
A flat/declining average means the pattern is exhausting — catching
the tail of a signal adds losses, not returns.  The XOR trend gate
is unchanged; only U1 itself gets the momentum filter.
...
def compute_signals(comp: pd.DataFrame, entry_mode: str):
    """Return dict of signal arrays. entry_mode: 'current' or 'idea2'."""
    N       = len(comp)
    ...
    # Idea 2: momentum filter — ehma3 must be rising vs prior bar
    ehma3_rising = np.zeros(N, dtype=bool)
    ehma3_rising[1:] = ehma3[1:] > ehma3[:-1]
    ...
    # XOR trend gate is unchanged for both modes
...
def main():
    print("\n" + "═"*70)
    print("  Idea 2 Comparison — MSTR & MSTU (all UI periods)")
    print("  U1 filter: require ehma3 rising (today > yesterday)")
    print("═"*70)
    ...
    for asset_label, ticker, stop_pct, periods in configs:
        ...
        for period_name, (end_iso, start_iso) in periods.items():
            ...
            r_i2  = run_asset_backtest(preds_df, raw_df, ticker, start_iso, end_iso, stop_pct, "idea2")
            if r_cur and r_i2:
                delta_ret = r_i2["strat_ret"] - r_cur["strat_ret"]
                delta_dd  = r_i2["max_dd"]    - r_cur["max_dd"]
                delta_sh  = r_i2["sharpe"]    - r_cur["sharpe"]
                ...
                print(f"  {'':8}  {'Δ idea2':<9}"
                      ...
                      f"  {'':>6}"
                      f"  {r_i2['n_trades']-r_cur['n_trades']:>+4}"
                      f"  {r_i2['n_sl']-r_cur['n_sl']:>+4}")
            print()
