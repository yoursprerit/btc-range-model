#!/usr/bin/env python3
"""
Offline verification of V5 (Idea 4: U1 persistence filter) vs V1 baseline.

Key correctness fixes vs previous offline scripts:
  1. Each period fetches its OWN dataset restricted to [warmup_start, bt_end]
     — no period leaks beyond its end date.
  2. Backtest loop exactly matches app's if/else structure (no same-bar re-entry).
...
def compute_sigs(comp):
    ...
    # V5: U1 persistence — require u1 on both bar i and i-1
    u1p = np.zeros(N,dtype=bool); u1p[1:] = u1[1:] & u1[:-1]
    tf1_v5 = u1p & ((above_ma30 ^ c10) | vr)
    return dict(N=N, dates=comp.index, ca=ca, above_ma30=above_ma30,
                bull=bull, vr=vr, d1=d1, d2=d2, d3=d3,
                u1=u1, u1p=u1p, tf1_v1=tf1_v1, tf1_v5=tf1_v5)

def run_bt(sigs, bt_start_iso, bt_end_iso, asset_px, sl_pct, label=""):
    """
    SL5 loop exactly matching app structure.
    - Restricted to [bt_start, bt_end] — no leakage beyond end date.
    - No same-bar re-entry: if/else ensures LONG block and CASH block never both run.
    """
    ...
    _btN = int(np.sum(_end_mask))   # last valid index + 1
...
# ── synthetic MSTU for bull period ────────────────────────────────────────────
...
print("\n" + "="*70)
print("Running per-period backtests …")
...
for label, warmup_start, bt_start, bt_end, use_synth in PERIODS:
    ...
    for asset, v1, v5 in [("MSTR", rv1_mstr, rv5_mstr), ("MSTU", rv1_mstu, rv5_mstu)]:
        d = v5["strat_ret"] - v1["strat_ret"]
    ...
    v5_t = {(pd.Timestamp(t["entry_date"]).date() if not isinstance(t["entry_date"], pd.Timestamp)
              else t["entry_date"].date()): t for t in rv5_mstr["trades"]}
    blocked_trades = [(d, t) for d, t in v1_t.items() if d not in v5_t]
    if blocked_trades:
        for d, t in sorted(blocked_trades):
            print(f"    {d}  trig={t['entry_trigger']:<12}  P&L={t['pnl_pct']:+.1f}%  exit={t['exit_signal']}")
    else:
        print("    (none)")
...
for label, *_ in PERIODS:
    r = all_results[label]
    for asset, v1, v5 in [("MSTR", r["v1_mstr"], r["v5_mstr"]),
                           ("MSTU", r["v1_mstu"], r["v5_mstu"])]:
        d = v5["strat_ret"] - v1["strat_ret"]
        print(f"{label:<22} {asset:<6} {v1['bh_ret']:>+7.1f}% {v1['strat_ret']:>+7.1f}% "
              f"{v5['strat_ret']:>+7.1f}% {d:>+6.1f}pp  {v1['win_rate']:>6.0f}%  "
              f"{v5['win_rate']:>6.0f}%  {v1['n_trades']:>5d}  {v5['n_trades']:>5d}  "
              f"{v1['max_dd']:>6.1f}%  {v5['max_dd']:>6.1f}%")
print("\nDone.")
