"""Idea 4 Entry Signal Comparison — MSTR & MSTU

Compares the CURRENT entry gate:
    tf1_entry = u1 & ((above_ma30 ^ clean_10d) | v_recent)

vs IDEA 4 entry gate (deep-bear V-reversal only gate):
    deep_bear = ~above_ma30 & ~ma30_slope_pos      # below AND declining MA30
    tf1_entry = u1 & (
        (~deep_bear & (above_ma30 ^ clean_10d)) |  # normal gate outside deep bear
        v_recent                                    # V-reversal always allowed
    )

In deep bear territory (price below a declining MA30) regular U1 entries
are blocked entirely — only capitulation V-reversals may trigger entry.
Outside deep bear the current XOR gate is unchanged.  Bull market entries
are completely unaffected because deep_bear is False throughout.
...
def compute_signals(comp: pd.DataFrame, entry_mode: str):
    """Return dict of signal arrays. entry_mode: 'current' or 'idea4'."""
    N       = len(comp)
    ...
    if entry_mode == "idea4":
        # Idea 4: in deep bear (below AND declining MA30), only V-reversals allowed
        deep_bear = ~above_ma30 & ~ma30_slope_pos
        tf1_entry = u1 & (
            (~deep_bear & (above_ma30 ^ clean_10d)) |
            v_recent
        )
    else:
        # Current / baseline
...
def main():
    print("\n" + "═"*70)
    print("  Idea 4 Comparison — MSTR & MSTU (all UI periods)")
    print("  Deep-bear gate: V-reversal only when below declining MA30")
    print("═"*70)
    ...
    for asset_label, ticker, stop_pct, periods in configs:
        ...
        for period_name, (end_iso, start_iso) in periods.items():
            ...
            r_i4  = run_asset_backtest(preds_df, raw_df, ticker, start_iso, end_iso, stop_pct, "idea4")
            if r_cur and r_i4:
                delta_ret = r_i4["strat_ret"] - r_cur["strat_ret"]
                delta_dd  = r_i4["max_dd"]    - r_cur["max_dd"]
                delta_sh  = r_i4["sharpe"]    - r_cur["sharpe"]
                ...
                print(f"  {'':8}  {'Δ idea4':<9}"
                      ...
                      f"  {'':>6}"
                      f"  {r_i4['n_trades']-r_cur['n_trades']:>+4}"
                      f"  {r_i4['n_sl']-r_cur['n_sl']:>+4}")
            print()
