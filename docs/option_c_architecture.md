...
| 11 | 💻 Laptop 🛡️ | `target_book.validate()` · `ibkr_common.is_trading_day()` · `bar_is_current()` · `eb.completed_run()` | Reject a stale signal bar or a book generated too long ago; require the book's equity basis to be the last completed session (a weekend `as_of` over it is fine); refuse a bar already executed; skip weekends and US holidays. |
