"""XLE / OIH / ERX — retrain on the 2020→2025 window, tune thresholds for 2021→today.

Third variant in the retrain study.  The training data is now a RECENT WINDOW
2020-01-01 → 2025-12-31 (~6 yrs), rather than the committed early window
(2015→2020) or the full history (2015→2025).  The window's left edge sits inside
the 2020 COVID / oil-war crash, so the "exclude drastic XLE drops" lever matters
most here.

  * Re-fit the XLE daily High/Low signal model on [2020-01-01, 2025-12-31], with
    and without excluding drastic XLE down-days.
  * Sweep the divergence thresholds against the 2021→today objective window.
  * Back-test XLE / OIH / ERX across every UI period (some now in-sample) and
    compare with the currently-implemented strategy.

OIH & ERX trade the XLE signal, so the one XLE signal model drives all three.
...
OOS = ("2021-01-01", None)          # tuning objective window (2021 → today)
...
def main():
    ...
    print(f"Training window: {TRAIN_START} → {TRAIN_END}  |  "
          f"objective: maximise return / minimise losses over 2021 → today\n")

    # committed baseline — train ≤ 2020-12-31 (full history back-start), live thresholds
    ...
    print(f"BASELINE (committed, train 2015→2020): XLE 2021→now = "
          f"{fmt(bmx['strategy'])}  n={bmx['n_trades']}")

    # ── Section A: fixed committed thresholds — compare training WINDOWS ──────
    print("\n" + "=" * 96)
    print("SECTION A — committed thresholds FIXED, compare training windows (metrics on 2021→now)")
    print("=" * 96)
    windows = [
        ("2015→2020 (committed)", None, "2020-12-31", None),
        ("2015→2025 (full)",      None, "2025-12-31", None),
        ("2020→2025",             "2020-01-01", "2025-12-31", None),
        ("2020→2025 drop<-7%",    "2020-01-01", "2025-12-31", -0.07),
        ("2020→2025 drop<-5%",    "2020-01-01", "2025-12-31", -0.05),
        ("2020→2025 drop<-4%",    "2020-01-01", "2025-12-31", -0.04),
    ]
    print(f"  {'train window':24s} {'rows':>5s} | {'XLE 2021→now':>24s} | {'OIH':>9s} | {'ERX':>9s}")
    for name, ts, te, db in windows:
        p, n = build_preds(daily, te, drop_below=db, train_start=ts)
    ...
    # ── retrain scenarios on the 2020→2025 window ────────────────────────────
    scenarios = {
        "w2020_full":   dict(train_start=TRAIN_START, train_end=TRAIN_END, drop_below=None),
        "w2020_no<-7%": dict(train_start=TRAIN_START, train_end=TRAIN_END, drop_below=-0.07),
        "w2020_no<-5%": dict(train_start=TRAIN_START, train_end=TRAIN_END, drop_below=-0.05),
        "w2020_no<-4%": dict(train_start=TRAIN_START, train_end=TRAIN_END, drop_below=-0.04),
    }
    cache = {}
    ...
    print(f"\n=== 2021→now sweep on 2020→2025 window: {len(rows)} configs, "
          f"{len(valid)} MDD≤B&H ===")
    print(f"  {'scen':13s} {'U1':>5s} {'D2':>6s} {'d1x':>4s} {'stop':>5s} "
          f"{'2021ret':>9s} {'MDD':>7s} {'Sh':>5s} {'trd':>4s} bear")
    for r in pool[:12]:
        print(f"  {r['scen']:13s} {r['U1']:5.2f} {r['D2']:6.2f} {str(r['d1_exit'])[:1]:>4s} "
    ...
    for r in by_sharpe[:5]:
        print(f"  {r['scen']:13s} U1={r['U1']:.2f} D2={r['D2']:.2f} d1x={str(r['d1_exit'])[:1]} "
    ...
    print("  (H/L trained on 2020→2025; thresholds tuned to 2021→now)")
    print("=" * 100)
    for lbl, col in TRADED:
        print(f"\n{lbl}:")
    ...
    def meta(pick, res):
        return dict(scenario=pick["scen"], train_window=[TRAIN_START, TRAIN_END],
    ...
    Path("data/xle/retrain_2020_2025_eval.json").write_text(json.dumps(dict(
        ...
        baseline=dict(train_window=["2015", "2020-12-31"], thresholds=LIVE, results=base_res),
        retrain_RET=meta(ret_pick, ret_res), retrain_RISK=meta(risk_pick, risk_res),
        sweep_top=pool[:20]), indent=2, default=str))
    print("\nSaved → data/xle/retrain_2020_2025_eval.json")
