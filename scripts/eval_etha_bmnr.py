"""Do ETHA and/or BMNR earn a place in the strategy? (evaluation only — nothing shipped)

Reproduces ETHA_BMNR_STRATEGY_EVAL.md:

  Part 1 — trade ETHA (iShares Ethereum Trust) and BMNR (Bitmine Immersion, the
           ETH-treasury equity) on the BTC app's signals: the CT-divergence
           engine (both live gates, per-asset stop grid, exactly as MSTR/MSTU
           are traded) and the robust simple BTC>SMA trend rule.
  Part 2 — search a robust strategy on ETHA's own price action (SMA / dual-MA /
...
def main() -> int:
    print("Fetching ETHA / BMNR / ETH-USD …", flush=True)
    etha, bmnr, eth = px("ETHA"), px("BMNR"), px("ETH-USD")
    ...
    for name, a in (("ETHA", etha), ("BMNR", bmnr)):
    ...
    ct_e = run_ct_sleeve(st, etha, "ma", None)
    print("\nETHA CT-MA trade log:")
    ...
    # ═══ Part 2 — ETHA price-action grid + validation ═════════════════════
    print("\n" + "═" * 90 + "\nPART 2 — ETHA price-action strategy search + validation\n" + "═" * 90)
    for asset, src, nm, lag, top in ((etha, etha, "ETHA ← own price", 0, 10),
                                     (etha, eth, "ETHA ← ETH parent (1d lag)", 1, 6),
                                     (bmnr, bmnr, "BMNR ← own price", 0, 5),
                                     (bmnr, eth, "BMNR ← ETH parent (1d lag)", 1, 5)):
    ...
    print("\n5-fold CV (contiguous blocks) — ETHA:")
    bh_e = etha.pct_change().fillna(0.0)
    nav_e = ct_e["nav"][ct_e["nav"].index >= etha.index[0]]
    kfold_line("CT-MA (BTC signal)", nav_e.pct_change().fillna(0.0), bh_e)
    kfold_line("BTC>SMA30", simple_bt(etha, rule_sma(btc, 30))["strat"], bh_e)
    kfold_line("own SMA30", simple_bt(etha, rule_sma(etha, 30))["strat"], bh_e)
    kfold_line("ETH MOM60 (lag1)", simple_bt(etha, rule_mom(eth, 60), extra_lag=1)["strat"], bh_e)
    ...
    kfold_line("ETH DMA25/100 (lag1)",
               simple_bt(bmnr, rule_dma(eth, 25, 100), extra_lag=1)["strat"], bh_b)

    print("\nWalk-forward (expanding, best SMA n in-sample → next fold):")
    for asset, src, nm, lag in ((etha, etha, "ETHA own-SMA", 0),
                                (etha, eth, "ETHA ETH-SMA (lag1)", 1),
                                (etha, btc, "ETHA BTC-SMA", 0)):
    ...
    for nm, a, sig, lag in (("ETHA BTC>SMA30", etha, rule_sma(btc, 30), 0),
                            ("ETHA own SMA30", etha, rule_sma(etha, 30), 0)):
    ...
    for nm, sig in (("ETH B&H", None), ("own SMA30", rule_sma(eth, 30)),
                    ("BTC>SMA30", rule_sma(btc, 30))):
        m = metrics(eth) if sig is None else simple_bt(eth, sig)
    ...
    for nm, sig, lag in (("BTC>SMA40", rule_sma(btc, 40), 0),
                         ("ETH DMA25/100 (lag1)", rule_dma(eth, 25, 100), 1)):
    ...
    ETHA_S, BMNR_S = sleeve("ETHA", etha, "core"), sleeve("BMNR", bmnr, "beta")
    ETHA_SMA = sleeve("ETHA", etha, "core", mode="sma")
    ...
    obase = optimise(base, {})
    scen = {"+ETHA (CT)": (base + [ETHA_S], {"ETHA": "core"}, ["ETHA"]),
            "+BMNR (CT)": (base + [BMNR_S], {"BMNR": "beta"}, ["BMNR"]),
            "+ETHA+BMNR": (base + [ETHA_S, BMNR_S], {"ETHA": "core", "BMNR": "beta"},
                           ["ETHA", "BMNR"]),
            "+ETHA (own-SMA30)": (base + [ETHA_SMA], {"ETHA": "core"}, ["ETHA"])}
    ...
    print("\nMC-seed noise band (baseline vs +ETHA, neutral optimal):")
    for seed in (7, 8, 9):
        ob = optimise(base, {}, seed=seed)
        oe = optimise(base + [ETHA_S], {"ETHA": "core"}, seed=seed)
    ...
    for nm, res in (("baseline", base), ("+ETHA", base + [ETHA_S]),
                    ("+BMNR", base + [BMNR_S]), ("+both", base + [ETHA_S, BMNR_S])):
    ...
    rets_all = oc.returns_matrix(base + [ETHA_S, BMNR_S])
    print("\nStrategy-stream correlation of the new sleeves to the book:")
    print(rets_all.corr()[["ETHA", "BMNR"]].drop(index=["ETHA", "BMNR"]).round(2).T.to_string())
    print("\nDone.")
    return 0
